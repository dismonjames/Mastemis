...
public sealed class SettingsViewModel : ObservableObject
{
    ...
    public IReadOnlyList<string> Themes { get; } = ["System", "Light", "Dark"]; public ICommand LoadCommand { get; } public ICommand SaveCommand { get; }
    public string Theme { get => theme; set => SetProperty(ref theme, value); } public string Language { get => language; set => SetProperty(ref language, value); }
    public int EditorFontSize { get => editorFontSize; set => SetProperty(ref editorFontSize, Math.Clamp(value, 10, 28)); } public int TabSize { get => tabSize; set => SetProperty(ref tabSize, Math.Clamp(value, 2, 8)); }
    public int AutosaveSeconds { get => autosaveSeconds; set => SetProperty(ref autosaveSeconds, Math.Clamp(value, 2, 60)); } public bool ReducedMotion { get => reducedMotion; set => SetProperty(ref reducedMotion, value); } public bool CompactTables { get => compactTables; set => SetProperty(ref compactTables, value); }
