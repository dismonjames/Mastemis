...
public sealed class ProblemStudioViewModel : ObservableObject
{
    ...
    public ProblemStudioViewModel(IProblemDraftClient draftsClient, IProblemMasClient masClient, IProblemGenerationClient generationClient,
        ...
        ProblemPermissionViewModel permissions)
    {
        ...
        Tests = tests; Packages = packages; Permissions = permissions;
    ...
    private async Task LoadAsync(CancellationToken ct) => await RunAsync(async () => { if (SelectedDraft is null) return; var detailed = await draftsClient.GetAsync(SelectedDraft.Id, ct) ?? SelectedDraft; SelectedDraft = detailed; Metadata.Load(detailed); Statements.SetProblem(detailed.Id); Assets.SetProblem(detailed.Id); ReferenceSolution.SetProblem(detailed.Id); Tests.SetProblem(detailed.Id); Packages.SetProblem(detailed.Id, detailed.Version); Permissions.SetProblem(detailed.Id); var value = await masClient.GetAsync(detailed.Id, ct); Source = value?.Source ?? string.Empty; Status = value is null ? "No MAS source" : $"MAS revision {value.Revision}"; }).ConfigureAwait(true);
