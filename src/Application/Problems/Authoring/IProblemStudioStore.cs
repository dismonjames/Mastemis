...
public interface IProblemStudioStore
{
    ...
    Task StageInputsAsync(ProblemGenerationOperation operation, IReadOnlyList<(int Index, string Group, byte[] Input, string Hash)> tests,
        CancellationToken cancellationToken);
    Task<ProblemGenerationOperation> TransitionGenerationAsync(Guid operationId, GenerationOperationStatus status,
