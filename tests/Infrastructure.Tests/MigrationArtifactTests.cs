...
public sealed class MigrationArtifactTests
{
    ...
    public void Initial_production_migration_generates_complete_postgresql_script()
    {
        ...
        Assert.EndsWith("_AddReferenceSolutionRevisions", migrations[10], StringComparison.Ordinal);
        Assert.EndsWith("_ExpandProblemPackageExports", migrations[11], StringComparison.Ordinal);
        Assert.EndsWith("_ExpandProblemDraftMetadata", migrations[12], StringComparison.Ordinal);
        Assert.EndsWith("_ExpandProblemMasMetadata", migrations[13], StringComparison.Ordinal);
        ...
        Assert.Contains("CREATE TABLE reference_solution_revisions", script, StringComparison.Ordinal);
        Assert.Contains("CREATE TABLE reference_solution_sources", script, StringComparison.Ordinal);
    }
